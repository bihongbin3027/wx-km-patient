...
<script>
  ...
  import imMgr from "../IM/chatroom.js";
  ...
  import { saveToLocal, formatDate, urlParse } from "../utils/utils"
  import { global_ApiConfig, global_StoreConfig, APPSECRET } from "../common/config"
  import axios from "axios"
  import API from "../api/api"

  const { doctorId } = urlParse()
  ...
  export default {
    ...
    created () {
      document.title = "咨询室";
      if (window.appToken == undefined) {
        window.appToken = window.localStorage.getItem('appToken')
      }
      if (window.userInfo == undefined) {
        window.userInfo = JSON.parse(window.localStorage.getItem('userInfo'))
      }
    },
    mounted () {
      Toast.loading("请稍候...");
      ...
      this.room = new imMgr();
      this.$store.dispatch('getLoginInfo').then(() => {
        this.setupIM(() => {
          Toast.hide();
          this.$store.state.messages = [];
          this.subscribeEvent();
    ...
    computed: {
      ...mapState(["loginInfo", "mediaInfo", "roomInfo", "messages"]),
      EmotionPicData() {
    ...
    methods: {
      //聊天页面增加一条消息
      appendToMessages(msg, isNewMsg) {
        var message = this.messageWrapperToUIRealtimeMessage(this.messageWrapperToUIMessage(this.messageWrapperToIMMessage(msg)));
        //#region 处理是否消息是否显示到聊天记录中
        //历史消息直接显示
        if (!isNewMsg && message) {
          this.messages.push(message);
        }
        //发送时已经添加到聊天记录了，所以要过滤自己发送的记录
        else if (message && !message.isSend) {
          this.messages.push(message);
        }
        this.$store.commit('MESSAGE', this.messages)
        // 更新最后的消息
        this.room.groupMgr.GroupsInfo[msg.sess.id()].LastMsg = msg;
        this.room.groupMgr.GroupsInfo[msg.sess.id()].LastMsgTime = msg.time;
      },
      //在线界面显示时需要包装
      messageWrapperToUIMessage(msg) {
        let UIMsg = msg.fromAccount ? msg.fromAccount : this.room.loginInfo.identifier
        var user = this.room.memberMgr.formatUser(UIMsg, this.lang);
        var message = {
          avatar: user.avatar,//头像
          author: msg.fromAccountNick,//名称
          html: msg.toHtml(), //内容
          content: msg.content,
          isSend: msg.isSend,
          time: msg.time,
        ...
        if (msg.elems[0].type == "TIMCustomElem") {
          ...
          message.msgType = this.enumMsgType.MSGTYPE_TEXT
        }
        else if (msg.elems[0].type == "TIMImageElem") {
          message.msgType = this.enumMsgType.MSGTYPE_IMAGE
        }
        else if (msg.elems[0].type == "TIMFileElem") {
          message.msgType = this.enumMsgType.MSGTYPE_ATTACH;
          message.FileUrl = global_StoreConfig.UrlPrefix + "/Files/" + msg.elems[0].content.uuid;//msg.elems[0].content.downUrl
          message.FileSize = this.bytesToSize(msg.elems[0].content.size);
          message.FileName = msg.elems[0].content.name;
        }
        else if (msg.elems[0].type == "TIMSoundElem") {
          message.msgType = this.enumMsgType.MSGTYPE_VOICE
          ...
          message.voiceUrl = global_StoreConfig.UrlPrefix + "/Audios/" + msg.elems[0].content.uuid;
        }
        return message;
      },
      messageWrapperToUIRealtimeMessage(msg) {
        if (msg) {
          msg.getTime = function (index) {
            var msg = this;
            //第一条消息的时间
            if (index == 0) {
      ...
      setupIM(fn) {
        this.room.setup({
          ...
          accountType: this.loginInfo.AccountType,
          identifier: this.loginInfo.Identifier,
          userSig: this.loginInfo.UserSign
        }, fn);
      },
      // 聊天头像
      avatarFormat({ isSend }) {
