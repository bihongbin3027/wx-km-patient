...
import webim from "../common/webim/webim";
// import videoMgr from "./chatroom-VideoMgr.Agora.js" 
import eventBus from './eventBus'
// let TLSHelper = require('../common/webim/tsl-api')
//定义聊天室相关类
var module = (function (window, webim) {
  ...
  var groupMgr = (function () {
    ...
    groupMgr.prototype.getGroupUsersInfo = function (groupId, okCallBack, failCallBack) {
      ...
      if (self.GroupsInfo[groupId] && self.GroupsInfo[groupId].UsersInfo && self.GroupsInfo[groupId].UsersInfo.length>1)
      {
        if (okCallBack) {
          okCallBack(self.GroupsInfo[groupId].UsersInfo);
        }
      }
      else {
  ...
  var imMgr = (function () {
      ...
      window.tlsGetUserSig = function (res) {
        ...
        if (res.ErrorCode == TlsErrorCode.OK) {
          ...
          if (res.ErrorCode == TlsErrorCode.SIGNATURE_EXPIRATION) {
            ...
            Toast("[" + res.ErrorCode + "]" + res.ErrorInfo, 1500);
          }
        }
      }
    ...
    imMgr.prototype.setup = function (config, onInitCompleted) {
      ...
      //使用托管身份验证模式
        ...
        //     Toast('callBackUrl非法，请在index.html中将其修改为您的网站首页地址!', 1500);
        //     return;
        //   }
        ...
        if (msg.elems[0].content &&
          ...
          if (typeof (data) == "string") {
            if (rbrace.test(data)) {
              ...
              //自定义消息中的内容是JSON
              data = msg.elems[0].content.data = JSON.parse(data);
            }
          }
